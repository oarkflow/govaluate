...
*/
func NewEvaluableExpression(expression string, onlyUnexported ...bool) (*EvaluableExpression, error) {
	o := false
	if len(onlyUnexported) > 0 {
		o = onlyUnexported[0]
	}
	functions := make(map[string]ExpressionFunction)
	return NewEvaluableExpressionWithFunctions(expression, functions, o)
}
...
*/
func NewEvaluableExpressionWithFunctions(expression string, functions map[string]ExpressionFunction, onlyUnexported ...bool) (*EvaluableExpression, error) {
	o := false
	if len(onlyUnexported) > 0 {
		o = onlyUnexported[0]
	}
